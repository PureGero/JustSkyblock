...
public class WarpedForestGenerator extends BaseIslandGenerator {
    ...
    public void generate(Block center, Random random) {
        ...
        Material[] plants = new Material[] {Material.WARPED_ROOTS,
        ...
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
                ...
                center.getRelative(x, 0, z).setType(Material.WARPED_NYLIUM);
            }
        }
        ...
        if (random.nextInt(3) + 1 == 1) {
            for (int i = 0; i < 4; i++) {
                ...
                if (center.getRelative(x, 1, z).getType().equals(Material.AIR)) {
                    center.getRelative(x, 0, z).setType(Material.WARPED_WART_BLOCK);
                }
            }
        }
